...
class _ChlChatScreenState extends State<ChlChatScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Consumer<CreateChannelProvider>(builder: (context, value, child) {
        if (!channelProvider.getCreated) {
          return Scaffold(
            key: _key,
            appBar: PreferredSize(
              ...
              child: ChatAppBar(
                data: chlProvider.topicData,
                height: _sizeH,
              ),
            ),
            body: Consumer2<ChlProvider, ChatButtonProvider>(builder: (context, value1, value2, child) {
              return Padding(
                      ...
                      child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            if (index != value1.chatList.length - 1) {
                              return ItemChatList.chatItem(value1.chatList[index], profileProvider.getUserName, profileProvider.fn + value1.chatList[index].seq.toString(), value1.topicData.public.fn,
